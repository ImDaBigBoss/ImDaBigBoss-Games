	...
	check_for_win : function(board) {
		...
		for (var row = 0; row < board.length; row++) {
			...
			for (var col = 0; col < board[row].length; col++) {
				...
				if (selected !== 0) {
					found = (person != selected) ? 1 : found + 1;
				}
		...
		if (!winner) {
			for (col = 0; col < board[0].length-3; col++) {
				...
				for (row = 0; row < board.length-3; row++) {
					...
					if (first_val === board[row+1][col+1] && first_val === board[row+2][col+2] && first_val === board[row+3][col+3]) {
						winner = first_val;
