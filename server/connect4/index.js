...
server.app.get('/connect4', function(req, res){
	res.writeHead(302, {
		'Location': '/connect4/' + utils.generateHash(6)
	});

	res.end();
})

server.app.get('/connect4/:room([A-Za-z0-9]{6})', function(req, res) {
	res.sendFile(server.getClientPagePath("html/games/connect4.html"));
});
...
server.ws_server.on('connection', function (socket) {
	socket.on(game_prefix + 'join', function(data) {
		...
		socket.on(game_prefix + 'makeMove', function(data) {
			...
			if (data.hash = socket.hash && game.turn == socket.pid) {
				...
				if (move_made) {
					...
					socket.broadcast.to(socket.room).emit(game_prefix + 'move_made', {pid: socket.pid, col: data.col});
		...
		socket.on(game_prefix + 'my_move', function(data) {
			socket.broadcast.to(socket.room).emit(game_prefix + 'opponent_move', {col: data.col});
		})
